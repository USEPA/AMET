...
if (hourly_ozone_analysis == 'y') {
   ...
   for (m in 1:length(batch_query)) {
      ...
      for (i in 1:length(species_list)) {
         ...
         figdir 		<- paste(out_dir,sub_dir,"multiple_species",sep="/")
         ...
         system(mkdir_command)
         if (soccer_plot	== 'y') { try(source(run_script_command1)) }
      }
   }
}
...
if (aerosol_analysis == 'y') {
   ...
   for (m in 1:length(batch_query)) {
      species 		<- c("SO4","NO3","NH4","TNO3","EC","OC","TC","PM_TOT")
      dates 		<- batch_names[m]
      sub_dir 		<- batch_names[m]
      figdir 		<- paste(out_dir,sub_dir,"multiple_species",sep="/")
      mkdir_command 	<- paste("mkdir",figdir)
...
if (dep_analysis == 'y') {	
   ...
   for (m in 1:length(batch_query)) {
      species 		<- c("SO4_dep","NO3_dep","NH4_dep","Precip")
      dates		<- batch_names[m]
      sub_dir		<- batch_names[m]
...
if (AE6_analysis == 'y') {
   ...
   for (m in 1:length(batch_query)) {
      species 		<- c("Na","Cl","Fe","Al","Si","Ti","Ca","Mg","K","Mn","soil","NaCl","other","ncom","other_rem")
      dates 		<- batch_names[m]
      sub_dir 		<- batch_names[m]
      figdir         	<- paste(out_dir,sub_dir,"multiple_species",sep="/")
      mkdir_command  	<- paste("mkdir",figdir)
