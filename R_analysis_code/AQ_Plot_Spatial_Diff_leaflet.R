...
filename[5]          <- paste(figdir,filename_bias_diff,sep="/") 		# Filename for diff spatial plot
filename[3]          <- paste(figdir,filename_error_1,sep="/")     	# Filename for obs spatial plot
filename[4]          <- paste(figdir,filename_error_2,sep="/")     	# Filename for model spatial plot
...
### Match the points between each of the runs.  This is necessary if the data from each query do not match exactly ###
         ...
         sinfo_data[[j]]		<-list(stat_id=sites,lat=sites_avg.df$lat,lon=sites_avg.df$lon,Bias_1=sites_avg.df$Bias_1, Bias_2=sites_avg.df$Bias_2, Bias_Diff=sites_avg.df$Bias_Diff, Error_1=sites_avg.df$Error_1, Error_2=sites_avg.df$Error_2, Error_Diff=sites_avg.df$Error_Diff)
...
plot_data[[5]] <- all_bias_diff
plot_data[[6]] <- all_error_diff

title <- NULL
{
   if (custom_title == "") { 
      title[1] <- paste(run_name1, "<br>", dates, "<br>", species," (",units,") Bias",sep="") 
      title[2] <- paste(run_name2, "<br>", dates, "<br>", species," (",units,") Bias",sep="")
      title[5] <- paste(run_name1, " - <br>", run_name2, "<br>", dates, "<br>", species," (",units,") Bias Diff",sep="")
      title[3] <- paste(run_name1, "<br>", dates, "<br>", species," (",units,") Error",sep="")
      title[4] <- paste(run_name2, "<br>", dates, "<br>", species," (",units,") Error",sep="")
      title[6] <- paste(run_name1, "vs. <br>", run_name2, "<br>", dates, "<br>", species," (",units,") Error Diff",sep="")
   }
   else { 
...
#  o3.mod.raster <- rasterFromXYZ(xyz,crs="+proj=lcc +lat_1=33 +lat_2=45 +lat_0=40 +lon_0=-97 +a=6370000 +b=6370000")
  ...
  for (j in 1:length(network_names)) {
     ...
     contents <- paste("Site: ", all_sites,
                  ...
                  "<br/>",
                  "Error Diff:", round(sinfo_data[[j]]$Error_Diff, 2),units, sep=" ")
