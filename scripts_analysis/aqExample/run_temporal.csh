...
##--------------------------------------------------------------------------##
  ...
  setenv AMETRINPUT $AMETBASE/scripts_analysis/$AMET_PROJECT/temporal.input  
  ...
  R CMD BATCH --no-save --slave $AMETBASE/R/AQ_Temporal.R $AMET_LOG
  ...
  if($AMET_R_STATUS == 0) then
		...
		echo "-----------------------------------------------------------------------------------------"
		echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_ecdf.$AMET_PTYPE"
                echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_taylor.$AMET_PTYPE"
                echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_spectral.$AMET_PTYPE"
                echo "Plots -----------------------> $AMET_OUT/${AMET_PROJECT}_${AMET_AQSPECIES}_qq.$AMET_PTYPE"
		echo "-----------------------------------------------------------------------------------------"
		exit 0
  else
