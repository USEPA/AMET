...
outname_pdf <- paste(run_name1,species,"scatterplot_mtom.pdf",sep="_")  # Set filename for pdf format file
outname_png <- paste(run_name1,species,"scatterplot_mtom.png",sep="_")  # Set filename for png format file
{
   if (custom_title == "") { title <- paste(run_name1," vs ",run_name2," ",species," for ",start_date,end_date,sep=" ") }
...
for (j in 1:length(network_names)) {						# Loop through for each network
   ...
   qs1	<- paste("SELECT d.network,d.stat_id,s.lat,s.lon,DATE_FORMAT(d.ob_dates,'%Y%m%d'),DATE_FORMAT(d.ob_datee,'%Y%m%d'),d.ob_hour,d.month,d.",species,"_ob,d.",species,"_mod,precip_ob,precip_mod from ",run_name1," as d, site_metadata as s",criteria," ORDER BY stat_id,ob_dates",sep="")                                           # Secord part of MYSQL query for run name 1
   qs2       <- paste("SELECT d.network,d.stat_id,s.lat,s.lon,DATE_FORMAT(d.ob_dates,'%Y%m%d'),DATE_FORMAT(d.ob_datee,'%Y%m%d'),d.ob_hour,d.month,d.",species,"_ob,d.",species,"_mod,precip_ob,precip_mod from ",run_name2," as d, site_metadata as s",criteria," ORDER BY stat_id,ob_dates",sep="")                                           # Second par of MYSQL query for run name 2
   ...
   if (length(aqdat1.df$statdate) <= length(aqdat2.df$statdate)) {				# If more obs in run 1 than run 2
      ...
      aqdat.df<-data.frame(network=aqdat1.df$network, stat_id=aqdat1.df$stat_id, lat=aqdat1.df$lat, lon=aqdat1.df$lon, ob_dates=aqdat1.df$ob_dates, aqdat1.df[,10], aqdat2.df[match.ind,10], aqdat1.df[,9], aqdat2.df[match.ind,9], month=aqdat1.df$month, precip_ob=aqdat1.df$precip_ob, precip_mod=aqdat1.df$precip_mod)	# eliminate points that are not common between the two runs
   ...
   indic.na <- is.na(aqdat_stats.df$Obs_Value)		# Indentify NA records
   aqdat_stats.df <- aqdat_stats.df[!indic.na,]		# Remove NA records
...
y <- c(y1,y2,y3,y4,y5,y6,y7)			# Create vector of y offset values
...
text(x[2],y[6], paste(species," (",units,")"),cex=1.2,adj=0)		# add species text
text(x[2],y[7], aqdat.df$avg_text, cex=1.2, adj=0)			# add timescale text (e.g. monthly average)
...
plot_cols  <- c("red","blue","green","yellow","purple","brown","gray")	# vector of plot colors

for (i in 1:length(network_names)) {
   points(sinfo[[i]]$plotval_obs,sinfo[[i]]$plotval_mod,pch=plot_chars[i],col=plot_cols[i],cex=.8)	# add points to plot for each model/model pair
...
legend("topleft", network_label, pch=plot_chars,col=plot_cols, merge=F, cex=1.5, bty="n")
##############################
