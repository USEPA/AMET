...
############################################################################
### Set base query string (works a bit different from the other scripts) ###
############################################################################
query <- paste(" and s.stat_id=d.stat_id and ob_hour between", start_hour, "and", end_hour, add_query,sep=" ")
...
################################################
outname_stats	<- paste(run_name1,species,"stats.csv",sep="_")
outname_pdf1	<- paste(run_name1,species,"plot1.pdf",sep="_")
outname_png1	<- paste(run_name1,species,"plot1.png",sep="_")
outname_pdf2	<- paste(run_name1,species,"statsplot1.pdf",sep="_")
outname_png2	<- paste(run_name1,species,"statsplot1.png",sep="_")
outname_pdf3	<- paste(run_name1,species,"statsplot2.pdf",sep="_")
outname_png3	<- paste(run_name1,species,"statsplot2.png",sep="_")
## Create a full path to file 
outname_stats	<- paste(figdir,outname_stats,sep="/")
outname_pdf1	<- paste(figdir,outname_pdf1,sep="/")
outname_png1	<- paste(figdir,outname_png1,sep="/")
outname_pdf2	<- paste(figdir,outname_pdf2,sep="/")
outname_png2	<- paste(figdir,outname_png2,sep="/")
outname_pdf3	<- paste(figdir,outname_pdf3,sep="/")
outname_png3	<- paste(figdir,outname_png3,sep="/")

{
   if (custom_title == "") { title  <- paste(run_name1,network_name,species,"for",start_date,"to",end_date,"; State: ",state,"; Site: ",site,sep=" ") }
...
end_month	<- as.numeric(substr(end_date,5,6))
...
header <- c(paste("Run Name = ",run_name1,sep=""),paste("Evaluation Dates = ",start_date,"to",end_date,sep=""),paste("Species = ",species,sep=""),paste("State = ",state,sep=""),paste("Site Name = ",site,sep=""),paste("Network = ",network_name,sep=""),paste("Units = ",units,sep=""))
write.table(header, file=outname_stats, append=F, sep="," ,col.names=F, row.names=F)
write.table("",file=outname_stats, append=T, sep=",",col.names=F,row.names=F)
write.table(all_stats.df, file=outname_stats, append=T, sep=",",col.names=T,row.names=F)
###########################################
...
pdf(file=outname_pdf1,width=10,height=8)
...
### Add various text, legend and title to the plot ###
{
   if (length(months) <= 12) {	# If plotting multiple years, add vertical lines and year name to denote each year
      ...
      year_mark<-seq(13,max(months),by=12)	 # Do not plot number of obs if plotting more than a year
...
if (ametptype == "png") {
   ...
   convert_command<-paste("convert -density 150x150 ",outname_pdf1," ",outname_png1,sep="")
   system(convert_command)

#   ### Remove PDF ###
#   remove_command <- paste("rm ",outname_pdf1,sep="")
...
### Create empty plotting region with labels ###
pdf(file=outname_pdf2,width=10,height=8)
par(lab=c(12,10,10),mar=c(5,4,4,4))
...
if (ametptype == "png") {
   ### Convert pdf format file to png format ###
   convert_command<-paste("convert -density 150x150 ",outname_pdf2," ",outname_png2,sep="")
   system(convert_command)

#   ### Remove PDF ###
#   remove_command <- paste("rm ",outname_pdf2,sep="")
#   system(remove_command)
}
####################################
...
### Create empty plotting region with labels ###
pdf(file=outname_pdf3,width=10,height=8)
par(lab=c(12,10,10),mar=c(5,4,4,4))
...
if (ametptype == "png") {
   ### Convert pdf format file to png format ###
   convert_command<-paste("convert -density 150x150 ",outname_pdf3," ",outname_png3,sep="")
   system(convert_command)

#   ### Remove PDF ###
#   remove_command <- paste("rm ",outname_pdf3,sep="")
#   system(remove_command)
}
####################################
