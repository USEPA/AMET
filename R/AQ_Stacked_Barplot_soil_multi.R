...
outname_pdf <- paste(run_name1,"soil_stacked_barplot.pdf",sep="_")
outname_png <- paste(run_name1,"soil_stacked_barplot.png",sep="_")
...
{
   if (custom_title == "") { title <- paste("Soil Stacked Barplot (",method,") for",run_name1,"for",start_date,"to",end_date,sep=" ") }
...
filename_txt <- paste(run_name1,species,"stacked_barplot_soil.csv",sep="_")     # Set output file name
...
if (run_info_text == "y") {
   ...
   if (site != "All") {
      text(2.07,yaxis.max*.52,paste("Site: ",site,sep=""),cex=1,adj=c(1,0))
   }
   if (state != "All") {
      text(2.07,yaxis.max*.52,paste("State: ",state,sep=""),cex=1,adj=c(1,0))
   }
} 
############################################################
