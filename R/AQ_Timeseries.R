...
### averaged across space to create the timeseries plot, and mutiple runs.
### In addition, the script also allows the user to specify the 
### temporal averaging to use. For example, hourly data can be averaged
### and plotted as daily or even monthly values. If no averaging is
### specified in the input file, the default temporal frequency of the
### data is used as the plotting frequency. The script also plots the 
### bias between the obs and model.
###
...
filename_pdf <- paste(run_name1,network,species,site,"timeseries.pdf",sep="_")              # Set output file name
filename_png <- paste(run_name1,network,species,site,"timeseries.png",sep="_")
filename_txt <- paste(run_name1,network,species,site,"timeseries.csv",sep="_")
...
for (j in 1:num_runs) {	# For each simulation being plotted
   ...
   write.table(run_names[j],file=filename_txt,append=T,row.names=F,sep=",")	# Write header for raw data file
...
if (num_runs > 1) {
   for (k in 2:num_runs) {
      ...
      if (length(Mod_Mean[[1]]) < 150) {
         points(Dates[[k]],Mod_Mean[[k]],col=plot_colors[k])
      }
...
text(max(Dates[[1]]),ymax,paste("# of Sites: ",num_sites,sep=""),pos=2,cex=1)
...
text(max(Dates[[1]]),bias_max,paste("# of Sites: ",num_sites,sep=""),pos=2,cex=1)
legend("topleft",legend=legend_names,col=legend_colors,lty=c(1,1,1), merge=TRUE,cex=1, bty='n')  # Add legend
