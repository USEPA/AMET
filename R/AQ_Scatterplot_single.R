...
### single network, up to two runs may be used.
###
...
outname_pdf <- paste(run_name1,species,"scatterplot_single.pdf",sep="_")                                # Set PDF filename
outname_png <- paste(run_name1,species,"scatterplot_single.png",sep="_")                                # Set PNG filename
...
for (j in 1:num_runs) {
   ...
   qs <- paste("SELECT d.network,d.stat_id,s.lat,s.lon,d.ob_dates,d.ob_datee,d.ob_hour,d.month,d.",species,"_ob,d.",species,"_mod, precip_ob, precip_mod from ",run_name[j]," as d, site_metadata as s",criteria," ORDER BY network,stat_id",sep="") # Set the rest of the MYSQL query
   ...
   ##############################
   if (j == 1) {
      filename_txt <- paste(run_name1,species,"scatterplot_single.csv",sep="_")     # Set output file name
...
#         plot_chars <- c(7,8,9,10)                                            # set vector of plot characters
   ...
   if (k == 1) {
      x_stats <- c(axis.max*0.01,axis.max*0.24,axis.max*0.10,axis.max*0.33,axis.max*0.20,axis.max*0.42,axis.max*0.07,axis.max*0.32)
      y_stats <- NULL
      ...
      x_stats <- c(axis.max*0.61,axis.max*0.84,axis.max*0.70,axis.max*0.93,axis.max*0.80,axis.max*1.02,axis.max*0.67,axis.max*0.92,axis.max*0.83)
      y_stats <- NULL
