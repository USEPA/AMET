...
outname_pdf <- paste(run_name1,species,"stacked_barplot.pdf",sep="_")
outname_png <- paste(run_name1,species,"stacked_barplot.png",sep="_")
...
{
   if (custom_title == "") { title <- paste(network_label," Stacked Barplot (",method,") for ",run_name1," for ",start_date, "to", end_date, sep="") }
...
m <- 9							# offset for first species mod value
...
if (num_runs > 1) {
   ...
   m <- 9                                          # offset for first specie mod value
...
###################################################################
### Calculate percent of total that each soil species comprises ###
###################################################################
...
filename_txt <- paste(run_name1,species,"stacked_barplot.csv",sep="_")     # Set output file name
...
if (num_runs > 1) {
   ...
   text(x2,(medians2.df$Fe_mod+medians2.df$Al_mod+medians2.df$Ti_mod+medians2.df$Si_mod+medians2.df$Ca_mod+medians2.df$Mg_mod+medians2.df$K_mod+(medians2.df$Mn_mod)+(medians2.df$Cl_mod/2)),paste(Cl_mod_percent2,"% --",sep=""),cex=1.2,adj=c(1,0.5))
   text(x2,(medians2.df$Fe_mod+medians2.df$Al_mod+medians2.df$Ti_mod+medians2.df$Si_mod+medians2.df$Ca_mod+medians2.df$Mg_mod+medians2.df$K_mod+(medians2.df$Mn_mod)+(medians2.df$Cl_mod)+(medians2.df$Na_mod/2)),paste(Na_mod_percent2,"% --",sep=""),cex=1.2,adj=c(1,0.5))
}
##########################################################################
