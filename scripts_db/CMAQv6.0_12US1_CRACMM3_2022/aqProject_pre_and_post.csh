...
#> in the script.
 ...
 set CMAQ_HOME = ~/CMAQv6

#> Base directory where AMET code resides
 setenv AMETBASE	~/AMET_v16
...
#> Start and end dates of simulation to be evaluated.
 setenv START_DATE_H  "2022-01-01"              #> Start day. Should be in format "YYYY-MM-DD".
 setenv END_DATE_H    "2022-02-28"              #> End day. Should be in format "YYYY-MM-DD".

#> Set General Parameters for Configuring the Simulation
 set VRSN      = v6               #> Code Version
 set PROC      = mpi               #> serial or mpi
 set MECH      = cracmm3      #> Mechanism ID
 set APPL      = v6_intel_2022hd_CRACMM3_12US1_BASE         #> Application Name (e.g. Gridname)
...
#> for example: CCTM_ACONC_v6_intel_12NE3BENCH_${YYYY}${MM}${DD}.nc
...
#> ${COMBINE_ACONC_NAME}_${YYYY}${MM}.nc,${COMBINE_DEP_NAME}_${YYYY}${MM}.nc
 ...
 setenv POSTDIR   ~/AMET_v16/model_data/AQ/CMAQv6.0_12US1_CRACMM3_2022

 set COMBINE_AELMO_NAME  = COMBINE_AELMO_${RUNID} #> Name of combine AELMO file (without date and file extension).
 set COMBINE_DEP_NAME    = COMBINE_DEP_${RUNID}   #> Name of combine DEP file (without date and file extension).
 set HR2DAY_AELMO_NAME   = dailymaxozone_${RUNID}  #> Name of hr2day file (without date and file extension).

#> If data for January of the following year exists, set paths and files names here
 # set APPL2 = v6_cb6r5_ae7_aq_WR413_MYR_STAGE_2018_12US1
 # setenv POSTDIR2 /work/MOD3EVAL/wtt/EQUATES/data/output_v6_cb6r5_ae7_aq_WR413_MYR_STAGE_2018_12US1/PostProcess
 ...
 # set HR2DAY_ACONC_NAME2  = dailymaxozone_${APPL2}                      #> Name of HR2DAY ACONC file (without date and file extension).
...
#> Project name and details. Project will be created if it does not already exist.
 ...
 setenv RUN_DESCRIPTION "CMAQv6_12US1_2022"
...
#> Loop through all days between START_DAY and END_DAY
 ...
 while ($TODAYJ <= $STOP_DAY )  #>Compare dates in terms of YYYYJJJ
  ...
  #> A new file will be created for each month/year.
   setenv OUTFILE ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYY}${MM}.nc
  ...
  #> File [4]: MCIP METCRO2D file
   setenv INFILE1 ${CCTMOUTDIR}/${CCTM_AELMO_NAME}_${YYYY}${MM}${DD}.nc
...
##> Loop through all months between START_DAY and END_DAY
 ...
 while ($TODAYJ <= $STOP_DAY )  #>Compare dates in terms of YYYYJJJ
  ...
  #> Retrieve Calendar day information
   ...
   setenv M3_FILE_1 ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYY}${MM}.nc
   setenv OUTFILE ${POSTDIR}/${HR2DAY_AELMO_NAME}_${YYYY}${MM}.nc
   echo "M3_FILE_1",$M3_FILE_1
  ...
  #> Check to see if the HR2DAY AELMO file for the next month exists.
  ...
  #> after the time zone shift to LST.
   if(-e ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYYnext}${MMnext}.nc) then
     setenv M3_FILE_2 ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYYnext}${MMnext}.nc
...
#> Convert startg, endg and END_DATE from YYYY-MM-DD to YYYYJJJ
 ...
 while (${startj} <= ${stopj} )  #>Compare dates in terms of YYYYJJJ
  ...
  #> AELMO and DEP combine files for this month and the following month.
   setenv CONC_FILE_1 ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYY}${MM}.nc
   setenv DEP_FILE_1 ${POSTDIR}/${COMBINE_DEP_NAME}_${YYYY}${MM}.nc
   setenv HR2DAY_FILE_1 ${POSTDIR}/${HR2DAY_AELMO_NAME}_${YYYY}${MM}.nc
  ...
  #>
   if ((${YYYYnext} > ${YYYY}) && ($?POSTDIR2)) then
     ...
     set COMBINE_AELMO_NAME = ${COMBINE_AELMO_NAME2}
     set COMBINE_DEP_NAME = ${COMBINE_DEP_NAME2}
     set HR2DAY_AELMO_NAME = ${HR2DAY_AELMO_NAME2}
   endif

  #> Check to see if the combine AELMO file for the next month exists.
  #> 
   if(-e ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYYnext}${MMnext}.nc) then
     setenv CONC_FILE_2 ${POSTDIR}/${COMBINE_AELMO_NAME}_${YYYYnext}${MMnext}.nc
   endif
...
#  #> Check to see if the combine DEP file for the next month exists.
   if(-e ${POSTDIR}/${HR2DAY_AELMO_NAME}_${YYYYnext}${MMnext}.nc) then
     setenv HR2DAY_FILE_2 ${POSTDIR}/${HR2DAY_AELMO_NAME}_${YYYYnext}${MMnext}.nc 
   endif
